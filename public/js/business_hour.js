...
var BusinessHoursController = /*#__PURE__*/function () {
  ...
  _createClass(BusinessHoursController, [{
    ...
    value: function addBusinessHour(addBtn) {
      ...
      if (this.countBusinessHours(dayOfWeek) === 5) document.querySelector("#btn_add".concat(dayOfWeek)).disabled = true;
    }
    ...
    value: function validate(event) {
      ...
      if (invalidBusinessHours.length > 0) {
        invalidBusinessHours.forEach(function (invalidBusinessHour) {
          return _this2.showErrorMessage(document.querySelector("#business_hour".concat(invalidBusinessHour)), 'businessHour');
        });
        isError = true;
      }
      for (var i = 0; i <= 6; i++) {
        if (this.validateTimeSeries(i) === false) {
          this.showErrorMessage(document.querySelector("#time__item".concat(i)), 'timeSeries');
          isError = true;
        }
      }
    ...
    value: function validateTimeSeries(dayOfWeek) {
      ...
      var result = true;
      for (var i = 1; i <= 5; i++) {
        if (document.querySelector("input[name=\"business_hour_id".concat(dayOfWeek, "-").concat(i, "\"]")) === null) continue;
        startHour = document.querySelector("select[name=\"start_hour".concat(dayOfWeek, "-").concat(i, "\"]")).value;
        startMinute = document.querySelector("select[name=\"start_minute".concat(dayOfWeek, "-").concat(i, "\"]")).value;
        endHour = document.querySelector("select[name=\"end_hour".concat(dayOfWeek, "-").concat(i, "\"]")).value;
        endMinute = document.querySelector("select[name=\"end_minute".concat(dayOfWeek, "-").concat(i, "\"]")).value;
      ...
      for (var j = 0; j < targetBusinessHours.length; j = j + 2) {
