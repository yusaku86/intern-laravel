...
var BusinessHoursController = /*#__PURE__*/function () {
  ...
  _createClass(BusinessHoursController, [{
    ...
    value: function addBusinessHour(addBtn) {
      ...
      cloneElement.id = "business_hour_new".concat(newId);
      ...
      cloneBusinessHourId.name = "business_hour_id_new".concat(newId);
      cloneBusinessHourId.value = "new_".concat(newId);
    ...
    value: function getNewId(targetBusinessHours) {
      ...
      var countNew = 0;
      for (var i = 0; i < targetBusinessHours.children.length; i++) {
        if (targetBusinessHours.children[i].classList.contains('new-business_hour')) countNew++;
      }
      return "".concat(dayOfWeek, "-").concat(countNew + 1);
    }
    ...
    key: "countBusinessHours",
    value: function countBusinessHours(dayOfWeek) {
      var targetBusinessHours = document.querySelector("#business_hours".concat(dayOfWeek, " "));
      var result = 0;
      for (var i = 0; i < targetBusinessHours.children.length; i++) {
      ...
      if (result >= 5) {
        document.querySelector("#btn_add".concat(dayOfWeek, " ")).disabled = true;
      }
      return;
    }
    /**
    ...
    value: function deleteBusinessHour(deleteBtn) {
      ...
      var newName = document.querySelector("#business_hour".concat(targetId, " ")) == null ? '_new' : '';
      var deleteTarget = document.querySelector("#business_hour".concat(newName).concat(targetId, " "));

      // 削除対象のDB上のidを表すinputタグ(新規追加分はinputタグのnameが business_hour_id_new${targetId})
      newName = document.querySelector("input[name=\"business_hour_id".concat(targetId, "\"]")) == null ? '_new' : '';
      var businessHourId = document.querySelector("input[name=\"business_hour_id".concat(newName).concat(targetId, "\"]"));
      ...
      if (businessHourId.value.substr(0, 4) !== 'new_') {
        ...
        var form = document.querySelector('#form_business_hour');
        form.action = "/business_hour/delete/".concat(businessHourId.value.split('-')[1]);
        form.submit();
        return;
      }
      ...
      // 「時間を追加」ボタンを有効化
      document.querySelector("#btn_add".concat(dayOfWeek, " ")).disabled = false;
    }
    ...
    value: function setHoliday(dayOfWeek) {
      ...
      // エラーメッセージがあれば非表示
      var nextElement = document.querySelector("#time__item".concat(dayOfWeek, " ")).nextElementSibling;
    ...
    value: function addChangedList(changedElement) {
      // 時間か分か
      var timeType = changedElement.classList.contains('time__item-hour') ? 'hour' : 'minute';
      // 変更された診療時間「曜日番号」-「DBのid」
      var businessHourId = changedElement.name.split(timeType)[1];
      // 変更された診療時間のリスト
    ...
    value: function validate(event) {
      ...
      // 開始時間と終了時間のバリデーション
      var invalidBusinessHoursNumber = this.validateBusinessHours();
      var invalidBusinessHour;
      var isError = false;

      // 開始時間と終了時間のバリデーションに引っかかったもののエラーメッセージ表示
      if (invalidBusinessHoursNumber.length > 0) {
        invalidBusinessHoursNumber.forEach(function (invalidBusinessHourNumber) {
          var _document$querySelect;
          invalidBusinessHour = (_document$querySelect = document.querySelector("#business_hour".concat(invalidBusinessHourNumber))) !== null && _document$querySelect !== void 0 ? _document$querySelect : document.querySelector("#business_hour_new".concat(invalidBusinessHourNumber));
          _this2.showErrorMessage(invalidBusinessHour, 'businessHour');
          isError = true;
        });
      }

      // 曜日内で診療時間が時系列に並んでいるかのバリデーション
      document.querySelectorAll('.time__item-business_hours').forEach(function (businessHourGroup) {
        if (_this2.validateTimeSeries(businessHourGroup) === false) {
          _this2.showErrorMessage(document.querySelector("#time__item".concat(businessHourGroup.id.slice(-1), " ")), 'timeSeries');
          isError = true;
        }
      });
      if (isError === true) {
    ...
    value: function validateTimeSeries(targetBusinessHourGroup) {
      var targetBusinessHours = [];
      var startHour;
      var startMinute;
      var endHour;
      var endMinute;
      var businessHourNumber;
      var result = true;
      Array.from(targetBusinessHourGroup.children).forEach(function (businessHour) {
        ...
        startHour = document.querySelector("select[name=\"start_hour".concat(businessHourNumber, "\"]")).value;
        startMinute = document.querySelector("select[name = \"start_minute".concat(businessHourNumber, "\"]")).value;
        endHour = document.querySelector("select[name = \"end_hour".concat(businessHourNumber, "\"]")).value;
        endMinute = document.querySelector("select[name = \"end_minute".concat(businessHourNumber, "\"]")).value;
      ...
      for (var i = 0; i < targetBusinessHours.length; i = i + 2) {
    ...
    value: function showErrorMessage(targetElement, option) {
      ...
      if (option === 'businessHour') {
        ...
        errorInput.name = "error_".concat(targetElement.id.replace('business_hour', '').replace('_new', ''));
      } else if (option === 'timeSeries') {
        errorP.innerHTML = '各曜日の診療時間は上から時系列で設定してください。';
        errorInput.value = '各曜日の診療時間は上から時系列で設定してください。';
        errorInput.name = "error_".concat(targetElement.id.slice(-1));
      }
